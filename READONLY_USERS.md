...
```bash
clickhouse-client -h localhost --user readonly_admin --password readonly_secure_2025
```

**ClickHouse Client (Native Protocol):**
```bash
clickhouse-client -h localhost --port 9000 --user readonly_admin --password readonly_secure_2025
```
...
curl -X POST 'http://localhost:8123' \
  ...
  -H 'X-ClickHouse-Key: readonly_secure_2025' \
...
```bash
clickhouse-client -h localhost --user readonly_admin --password readonly_secure_2025 << EOF
SELECT 
    transaction_id,
    amount,
    merchant,
