# PERFORMANCE OPTIMIZATIONS FOR KAFKA-CLICKHOUSE PIPELINE
# Save this as consumer_optimized.py to replace your current consumer.py
...
OPTIMAL_BATCH_SIZE = 1000  # Increased from 100
OPTIMAL_FLUSH_INTERVAL = 30    # Increased from 5 to 30 seconds
OPTIMAL_SESSION_TIMEOUT = 10000  # Reduced from 30000
MAX_RETRIES = 3
...
def log_malformed_message_batch(malformed_messages):
    ...
    try:
        with open(dlq_file, 'a') as f:
            f.write('\n'.join(batch_entries) + '\n')
        logger.info(f"📝 Batch logged {len(malformed_messages)} malformed messages to DLQ")
    except Exception as e:
        logger.error(f"❌ Failed to write to DLQ: {e}")

async def flush_to_clickhouse_async(batch):
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                lambda: client.insert(
                    ...
                    data_to_insert,
                    column_names=[
                        'transaction_id', 'account_id', 'sender_email', 
                        'recipient_email', 'amount', 'category', 
                        'merchant', 'transaction_type', 'event_time'
                    ]
                )
            )
...
def run_optimized_consumer():
    ...
    # Enable SASL/PLAIN authentication when credentials provided
    ...
    logger.info(f"✅ Consumer configured with batch_size={OPTIMAL_BATCH_SIZE}, flush_interval={OPTIMAL_FLUSH_INTERVAL}")
    ...
    try:
        while True:
            ...
            try:
                ...
                required_fields = ['transaction_id', 'account_id', 'amount', 'category', 'merchant', 'timestamp']
                ...
                # Flush conditions
                should_flush = (
                    len(buffer) >= OPTIMAL_BATCH_SIZE or
                    time.time() - last_flush_time >= OPTIMAL_FLUSH_INTERVAL
                )
                
                if should_flush:
                    logger.info(f"📊 Flushing {len(buffer)} messages (total processed: {processed_count})")
                    ...
                    asyncio.run(flush_to_clickhouse_async(buffer))
