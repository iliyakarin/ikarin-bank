...
def run_migration():
    ...
    client.command(f"CREATE DATABASE IF NOT EXISTS {os.getenv('CLICKHOUSE_DB')}")
    
    # Check if table exists
    table_exists = client.command(f"EXISTS TABLE {os.getenv('CLICKHOUSE_DB')}.transactions")
    
    if table_exists:
        # Add missing columns
        client.command(f"""
            ALTER TABLE {os.getenv('CLICKHOUSE_DB')}.transactions 
        ...
        client.command(f"""
            CREATE TABLE {os.getenv('CLICKHOUSE_DB')}.transactions (
