...
async def execute_admin_query(
    ...
    try:
        ...
        execution_time = (datetime.datetime.now(datetime.timezone.utc) - start_time).total_seconds() * 1000
    ...
    try:
        ...
        active_users_result = await db.execute(
            select(func.count(User.id)).filter(
                User.created_at >= datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1)
            )
        )
