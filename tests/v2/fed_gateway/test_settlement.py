...
def test_fednow_instant_settlement_success():
    """Verifies that FedNow payments are processed instantly with success."""
    engine = SettlementEngine()
    tx_id = "FEDNOW-1"
    amount = 1000.0
    
    result = engine.process_fednow_payment(tx_id, amount)
...
def test_fednow_insufficient_funds():
...
def test_fedwire_daylight_overdraft_success():
    ...
    result = engine.process_fedwire_payment(tx_id, amount)
    
    assert result["status"] == SettlementStatus.SETTLED.value
    assert engine.get_account_balance() == 5000000.0 - 5500000.0 # Becomes negative

def test_fedwire_exceed_overdraft_limit():
...
def test_invalid_amount():
    ...
    result = engine.process_fednow_payment(tx_id, -50.0)
    assert result["status"] == SettlementStatus.REJECTED.value
    
    result = engine.process_fedwire_payment(tx_id, 0.0)
    assert result["status"] == SettlementStatus.REJECTED.value
