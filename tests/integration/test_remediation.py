...
async def test_delete_user_full_cleanup(mock_fastapi_dependency):
    ...
    mock_ch = MagicMock()
    with patch('services.admin_service.get_ch_client', return_value=mock_ch), \
         patch('services.admin_service.emit_activity') as mock_emit, \
         patch('services.admin_service.CH_DB', "banking_log"):
